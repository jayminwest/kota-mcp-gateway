...
- If auth seems expired or you didn’t get a `refresh_token`:
  ...
  2) Delete `./data/whoop/tokens.json` (optional but ensures a clean slate).
  3) Visit `http://localhost:8081/auth/whoop/start` again.
  4) Accept consent when prompted (the gateway forces `prompt=consent` and requests the `offline_access` scope to obtain a refresh token).
